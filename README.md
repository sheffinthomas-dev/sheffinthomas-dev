...
**[LaraBoard Careers — AWS + Docker + Terraform + CI/CD](https://github.com/sheffinthomas-dev/laraboard-careers-devops)**
A production-style deployment of an open-source Laravel job board: containerized with Docker, provisioned on AWS EC2 entirely via Terraform, deployed automatically through GitHub Actions, secured with HTTPS, and monitored with Prometheus + Grafana. Includes a full write-up of real incidents I diagnosed and fixed — Docker build failures, a database race condition, stale Nginx IPs causing 502s, and a kernel OOM-killed MySQL process.
